...
class SearchBundleResponse(BaseModel):
    query: str
    movie_details: Optional[TMDBMovieDetails]=None
...
async def tmdb_search_movies(query: str, page: int = 1) -> Dict[str, Any]:
    """
    Raw TMDB response for keyword search
    """
...
def tfidf_recommend_for_new_movie(overview: str, top_n=10):
    if not overview or overview.strip() == "":
        return []

    new_vec = tfidf_obj.transform([overview])
    scores = (tfidf_matrix @ new_vec.T).toarray().ravel()
...
async def search_bundle(
    ...
    try:
        # 1. Try exact title from TMDB
        recs = tfidf_recommend_titles(details.title, top_n=tfidf_top_n)

    except Exception:
        try:
            # 2. Try user query (sometimes titles differ)
            recs = tfidf_recommend_titles(query, top_n=tfidf_top_n)

        except Exception:
            # 3. Runtime similarity for NEW movie
            if details.overview:
                recs = tfidf_recommend_for_new_movie(
                    details.overview,
                    top_n=tfidf_top_n
                )
            else:
